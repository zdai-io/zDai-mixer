...
const depositProver = spawn("node", ["depositProver.js"]);
depositProver.unref();
...
async function makeDeposit(contract, _account, _balance) {
  ...
  depositProver.stdin.write(JSON.stringify(stringifyBigInts(input)));
  const [pi_a, pi_b, pi_c, pubinputs] = JSON.parse(await new Promise(resolve => depositProver.stdout.on('data', resolve)));
  console.log(JSON.stringify([pi_a, pi_b, pi_c, pubinputs]));
  ...
  //console.log([hash, tx, pi_a, pi_b, pi_c, pubinputs]);
...
const main = async () => {
  ...
  //await makeDeposit(zDai, accounts[0], web3.utils.toWei("0.02"));
};
